...
def _get_master_user() -> Optional["LicMasterUser"]:
...
def require_master(f: Callable[..., Any]) -> Callable[..., Any]:
...
def _log_remote(user: Optional["LicMasterUser"], action: str, target_type: Optional[str] = None, target_id: Optional[int] = None, details: Optional[Dict[str, Any]] = None) -> None:
    ...
    try:
        ...
        log.debug("Failed to log remote activity: %s", action, exc_info=True)
        db.session.rollback()


# ── CLIENT ENDPOINTS (called by desktop clients) ────────────

_HEARTBEAT_THROTTLE: Dict[str, float] = {}
def _heartbeat_min_interval() -> int:
    try:
...
_HEARTBEAT_MIN_INTERVAL: int = 5  # default; evaluated per-request via _heartbeat_min_interval()


@remote_api_bp.route("/heartbeat", methods=["POST"])
def client_heartbeat() -> Any:
...
def list_clients() -> Any:
    ...
    cmd_cutoff = datetime.now(timezone.utc) - timedelta(hours=24)
...
@require_master
def get_client(client_db_id: int) -> Any:
...
@require_master
def send_command(client_db_id: int) -> Any:
...
@require_master
def update_config(client_db_id: int) -> Any:
...
@require_master
def suspend_client(client_db_id: int) -> Any:
...
@require_master
def reactivate_client(client_db_id: int) -> Any:
