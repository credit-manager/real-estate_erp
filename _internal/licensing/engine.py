...
def check_subscription(company_id: int) -> Dict[str, Any]:
...
def check_license(company_id: int) -> Dict[str, Any]:
...
def can_access(company_id: int) -> Dict[str, Any]:
...
def renew_subscription(subscription_id: int, days: int) -> Optional[LicSubscription]:
...
def suspend_license(company_id: int) -> Dict[str, Any]:
...
def revoke_license(company_id: int) -> Dict[str, Any]:
...
def check_user_limit(company_id: int) -> Dict[str, Any]:
    access = can_access(company_id)
    if not access["allowed"]:
    ...
    try:
        ...
        log.debug("Failed to count active users for company %d", company_id, exc_info=True)
        current = 0
