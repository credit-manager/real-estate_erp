...
from licensing.engine import can_access, renew_subscription, suspend_license, revoke_license, create_license
...
_registration_rate: Dict[str, List[float]] = defaultdict(list)
_REGISTRATION_RATE_LIMIT = 5  # max registrations per IP
_REGISTRATION_RATE_WINDOW = 3600  # 1 hour in seconds
...
def _check_registration_rate(ip: str) -> Tuple[bool, str]:
    ...
    _registration_rate[ip] = [t for t in _registration_rate[ip] if t > cutoff]
    if len(_registration_rate[ip]) >= _REGISTRATION_RATE_LIMIT:
        return False, "تم تجاوز الحد المسموح به من التسجيلات. يُرجى المحاولة لاحقاً."
    _registration_rate[ip].append(now)
    return True, ""


def _validate_registration_email(email: str) -> Tuple[bool, str]:
...
def _validate_registration_phone(phone: str) -> Tuple[bool, str]:
    """Validate phone format (optional field, but must be valid if provided)."""
    if not phone:
        return True, ""
    phone_re = r'^[\d\s\+\-\(\)]{7,20}$'
...
@admin_lic_bp.route("/register", methods=["GET"])
def register_page() -> str:
    """Serve the public company self-registration page."""
    from licensing.plans_data import TRIAL_DAYS
    return render_template("register.html", trial_days=TRIAL_DAYS)


@admin_lic_bp.route("/register", methods=["POST"])
def register_company() -> Any:
    """Public endpoint: allow a company to self-register for a trial.
    ...
    # ── Rate limit ──
    allowed, rate_msg = _check_registration_rate(ip)
    if not allowed:
    ...
    if not accept_terms:
        errors.append("يجب الموافقة على الشروط والأحكام")
    if errors:
        return jsonify({"success": False, "message": " | ".join(errors)}), 400

    # ── Email validation ──
    ok, msg = _validate_registration_email(admin_email)
    if not ok:
        return jsonify({"success": False, "message": msg}), 400

    # ── Phone validation ──
    ok, msg = _validate_registration_phone(phone)
    if not ok:
        return jsonify({"success": False, "message": msg}), 400

    # ── Password strength ──
    from utils.passwords import validate_password as _vp
    pw_ok, pw_msg = _vp(admin_password)
    if not pw_ok:
        return jsonify({"success": False, "message": pw_msg}), 400

    # ── Duplicate email check ──
    from licensing.models import LicCompanyUser
    if LicCompanyUser.query.filter_by(email=admin_email).first():
        return jsonify({
            "success": False,
            "message": "البريد الإلكتروني مسجل بالفعل. يُرجى تسجيل الدخول أو استخدام بريد آخر.",
        }), 400
    ...
    try:
        ...
        log.exception("Registration failed for %s", admin_email)
        return jsonify({"success": False, "message": "حدث خطأ أثناء التسجيل. يُرجى المحاولة لاحقاً."}), 500

    if not result["success"]:
        return jsonify({"success": False, "message": result.get("message", "فشل التسجيل")}), 500
    ...
    try:
        ...
        log.debug("Failed to log self-registration activity", exc_info=True)
        db.session.rollback()

    log.info("Self-registration: %s (company_id=%d, ip=%s)", admin_email, company.id, ip)

    return jsonify({
        "success": True,
        "message": "تم التسجيل بنجاح! بياناتك سترسل إلى بريدك الإلكتروني.",
        "company": {
            "id": company.id,
            "name": company.name,
            "email": company.email,
...
def _require_admin() -> Optional[LicMasterUser]:
...
def _admin_or_abort() -> Tuple[Optional[LicMasterUser], Optional[Any]]:
    user = _require_admin()
    if not user:
...
def _require_permission(permission_code: str) -> Optional[Any]:
...
def _log_activity(user: LicMasterUser, action: str, target_type: Optional[str] = None, target_id: Optional[int] = None, details: Optional[Dict[str, Any]] = None) -> None:
    try:
        log = LicActivityLog(
        ...
        log.debug("Failed to log activity: %s", action, exc_info=True)
        db.session.rollback()
...
def _company_users_count(company: LicCompany) -> int:
    ...
    try:
        ...
        log.debug("Failed to count users in company DB %d", company.id, exc_info=True)
        pass
...
def get_company(company_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
    perm_err = _require_permission("companies.view")
...
def renew_company(company_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
...
def upgrade_company(company_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
...
@admin_lic_bp.route("/companies/<int:company_id>/sale", methods=["POST"])
def sell_company_subscription(company_id: int) -> Any:
    """Record a paid sale (typically annual): renew/create subscription + confirm payment in one step."""
    user, err = _admin_or_abort()
    if err:
        return err
    perm_err = _require_permission("billing.payments")
    if perm_err:
        return perm_err
    ...
    try:
        days = int(data.get("period_days") or data.get("days") or 365)
    except (TypeError, ValueError):
        days = 365
    if days <= 0:
        return jsonify({"success": False, "message": "Sale period must be positive"}), 400
    ...
    if sub:
        ...
        if not plan:
            plan = LicPlan.query.filter_by(code="basic", is_active=True).first() or LicPlan.query.filter_by(code="basic").first()
            if not plan:
                return jsonify({"success": False, "message": "No applicable plan found"}), 500
        today = date.today()
        sub = LicSubscription(
        ...
        db.session.add(sub)
        db.session.flush()
        create_license(company.id, sub.id, days)
    ...
    if plan and sub.plan_id != plan.id:
        ...
        _log_activity(user, "subscription_plan_changed", target_type="subscription", target_id=sub.id,
                      details={"from": old_code, "to": plan.code})

    from datetime import datetime as _sale_dt
    currency = (data.get("currency") or "EGP").upper()
    payment = LicPayment(
        ...
        status="confirmed",
        paid_at=_sale_dt.utcnow(),
        confirmed_by=user.email,
    )
    db.session.add(payment)
    db.session.commit()

    _log_activity(user, "sale_recorded", target_type="company", target_id=company.id,
                  details={"days": days, "amount": amount_value, "currency": currency,
                           "plan": plan_code or (sub.plan.code if sub.plan else None) or None})

    lic = LicLicense.query.filter_by(company_id=company.id).order_by(LicLicense.id.desc()).first()
    return jsonify({
        "success": True,
        "subscription": sub.to_dict(),
        "payment": payment.to_dict(),
        "license": lic.to_dict() if lic else None,
        "message": f"تم تسجيل البيع: {days} يوم بقيمة {amount_value:g} {currency}",
    })


def _log_license_events(user: LicMasterUser, action: str, licenses: List[LicLicense]) -> None:
    for lic in licenses:
...
def suspend_company_action(company_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
...
def reactivate_company_action(company_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
...
def reset_company_admin_password(company_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
    perm_err = _require_permission("companies.edit")
...
def revoke_company_license(company_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
...
def license_detail(license_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
...
def update_company(company_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
    company = db.session.get(LicCompany, company_id)
...
def delete_company(company_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
    company = db.session.get(LicCompany, company_id)
...
def provision_company_db(company_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
...
def toggle_master_user(user_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
...
def renew_subscription_action(sub_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
    data = request.get_json(silent=True) or {}
...
def change_subscription_plan(sub_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
    data = request.get_json(silent=True) or {}
...
def cancel_subscription_action(sub_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
    sub = db.session.get(LicSubscription, sub_id)
...
def mark_payment_paid(payment_id: int) -> Any:
    user, err = _admin_or_abort()
    if err:
        return err
...
def reports() -> Any:
    ...
    revenue_by_month: Dict[str, float] = {m: 0.0 for m in months}
    companies_by_month: Dict[str, int] = {m: 0 for m in months}
    ...
    pending_total: float = sum(float(p.amount) for p in payments if p.status == "pending")

    top_by_rev: Counter[int] = Counter()
    for p in payments:
        if p.status != "confirmed":
            continue
...
def change_master_password() -> Any:
