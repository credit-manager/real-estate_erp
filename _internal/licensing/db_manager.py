...
def create_company_db(company: LicCompany) -> bool:
    ...
    try:
        ...
        registry = LicDatabaseRegistry(
            ...
            last_migration=datetime.now(timezone.utc), status="active",
        )
...
def get_company_engine(company: LicCompany) -> Engine:
...
def get_company_session(company: LicCompany) -> Session:
...
def get_db_size(company: LicCompany) -> float:
    try:
        ...
        log.debug("Failed to get DB size for company %d", company.id, exc_info=True)
        return 0.0
...
def archive_company_db(company: LicCompany) -> bool:
    try:
        ...
        new_name = f"archived_{old_name}_{datetime.now(timezone.utc).strftime('%Y%m%d')}"
        conn_str = f"postgresql://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}/postgres"
        engine = create_engine(conn_str, isolation_level="AUTOCOMMIT")
        with engine.connect() as conn:
