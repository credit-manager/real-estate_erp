...
SESS_COMPANY_ID: str = "lic_company_id"
SESS_COMPANY_USER_ID: str = "lic_company_user_id"
SESS_COMPANY_NAME: str = "lic_company_name"
SESS_COMPANY_DB_NAME: str = "lic_company_db_name"
SESS_COMPANY_ROLE: str = "lic_company_role"
SESS_COMPANY_USER_EMAIL: str = "lic_company_user_email"
SESS_COMPANY_FULL_NAME: str = "lic_company_full_name"
SESS_MASTER_USER_ID: str = "master_user_id"
SESS_MASTER_EMAIL: str = "master_user_email"
SESS_MASTER_NAME: str = "master_user_name"
SESS_MASTER_ROLE: str = "master_user_role"
SESS_MASTER_JTI: str = "master_jti"
SESS_MASTER_MFA_PENDING: str = "master_mfa_pending"
SESS_MASTER_MFA_ISSUED_AT: str = "master_mfa_issued_at"

COMPANY_SESSION_KEYS: list[str] = [SESS_COMPANY_ID, SESS_COMPANY_USER_ID, SESS_COMPANY_NAME, SESS_COMPANY_DB_NAME, SESS_COMPANY_ROLE, SESS_COMPANY_USER_EMAIL, SESS_COMPANY_FULL_NAME]
MASTER_SESSION_KEYS: list[str] = [SESS_MASTER_USER_ID, SESS_MASTER_EMAIL, SESS_MASTER_NAME, SESS_MASTER_ROLE, SESS_MASTER_JTI]
PENDING_MFA_KEYS: list[str] = [SESS_MASTER_USER_ID, SESS_MASTER_EMAIL, SESS_MASTER_MFA_PENDING, SESS_MASTER_MFA_ISSUED_AT]
MFA_PENDING_TTL_SECONDS: int = 300
MFA_PENDING_PATHS: set[str] = {"/admin/security/2fa/enroll", "/admin/security/2fa/verify", "/admin/security/2fa/verify-recovery"}


def clear_company_session() -> None:
...
def clear_pending_mfa_session() -> None:
    for key in PENDING_MFA_KEYS:
        session.pop(key, None)


def is_pending_mfa_session() -> bool:
...
def _mfa_required(user_id: int) -> bool:
    ...
    try:
        ...
        log.warning("MFA required check failed for user %d", user_id, exc_info=True)
        return False


def _mfa_enabled(user_id: int) -> bool:
    try:
        from security.two_factor import is_enabled
        return is_enabled(user_id)
    except Exception:
        log.warning("MFA enabled check failed for user %d", user_id, exc_info=True)
        return False


def _csrf_for_session() -> str:
...
def _establish_master_session(user: LicMasterUser) -> None:
    clear_pending_mfa_session()
...
def complete_pending_master_mfa(user_id: int) -> bool:
    ...
    try:
        ...
        log.warning("MFA completion failed for user %d", user_id, exc_info=True)
        db.session.rollback()
        session.clear()
        return False
    return True
...
def authenticate_company_user(email: str, password: str) -> Dict[str, Any]:
...
def logout_company_user() -> None:
    clear_company_session()


def authenticate_master_user(email: str, password: str) -> Dict[str, Any]:
...
def logout_master_user() -> None:
    _end_master_session()
    clear_master_session()
    clear_pending_mfa_session()


def _start_master_session(user: LicMasterUser, is_company_user: bool = False, extra: Optional[Dict[str, Any]] = None) -> str:
    from security.models import MasterSession
    jti = __import__("uuid").uuid4().hex
    sess = MasterSession(master_user_id=user.id, jti=jti, refresh_token_hash=None, ip=request.remote_addr, user_agent=(request.user_agent.string[:250] if request.user_agent else None), expires_at=datetime.now(timezone.utc) + timedelta(days=7), last_seen=datetime.now(timezone.utc))
    db.session.add(sess)
    return jti


def _end_master_session() -> None:
...
def get_master_session_data() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not jti or not master_session or (master_session.expires_at and master_session.expires_at < datetime.now(timezone.utc)):
            clear_master_session()
            return None
        user = db.session.get(LicMasterUser, uid)
        if not user or not user.is_active:
            clear_master_session()
            return None
        master_session.last_seen = datetime.now(timezone.utc)
        db.session.commit()
        return {"id": user.id, "email": user.email, "full_name": user.full_name or user.email, "role": user.role}
    except Exception:
        log.warning("Failed to get master session data for user %d", uid, exc_info=True)
        return None


def is_master_logged_in() -> bool:
    return get_master_session_data() is not None


def get_company_session_data() -> Optional[Dict[str, Any]]:
...
def is_company_user_logged_in() -> bool:
