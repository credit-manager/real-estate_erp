...
class Document(db.Model):
    ...
    def get_latest_version(self) -> Optional['Document']:
        """الحصول على أحدث إصدار."""
        if self.is_latest:
            return self
        return Document.query.filter_by(previous_version_id=self.id, is_latest=True).first()

    def create_new_version(self, file_path: str, file_name: str, file_size: int, mime_type: str, uploaded_by: int) -> 'Document':
...
def compute_file_hash(mapper, connection, target):
    if target.file_path and not target.file_hash:
        ...
        try:
            ...
            log.debug("Failed to compute file hash for %s", target.file_path, exc_info=True)
            pass