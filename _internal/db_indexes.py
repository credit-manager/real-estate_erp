...
def _validate_identifier(name: str) -> str:
    """Ensure name is a safe SQL identifier (alphanumeric + underscore only)."""
    import re
...
def ensure_indexes(
    ...
    Returns:
        Tuple of (created, skipped, errors) counts.
    """
    insp = inspect(engine)
    ...
    for table, cols in INDEXES.items():
        ...
        if tuple(cols) in existing[table]:
            ...
            continue
        try:
            # Validate all identifiers before constructing DDL
    ...
    try:
        ...
        log.debug("Failed to commit index DDL", exc_info=True)
        session.rollback()
