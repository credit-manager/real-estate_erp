...
def document_model(doc_type: str):
    """Get the SQLAlchemy model for a document type."""
...
def document_meta(doc_type: str, doc) -> Dict[str, Any]:
...
def _doc_amount(doc_type: str, doc) -> Optional[float]:
...
def active_template(doc_type: str, doc=None):
...
def user_is_approver(request) -> bool:
...
def submit_document_for_approval(doc_type: str, doc_id: int):
    ...
    db.session.commit()
    log_action("submit", "approval", req.id, f"{doc_type}:{doc_id}")
    return req


def cancel_document_approval(doc_type: str, doc_id: int) -> None:
...
def approve_request(req, comment: str = "") -> bool:
    ...
    if req.status == "approved":
        ...
        try:
            ...
            log_action("error", "approval", req.id, f"فشل الترحيل التلقائي: {e}")
    log_action("approve", "approval", req.id, f"{req.doc_type}:{req.doc_id}")
    return True


def cancel_request(req) -> bool:
    ...
    db.session.commit()
    log_action("cancel", "approval", req.id, f"{req.doc_type}:{req.doc_id}")
    return True


def reject_request(req, comment: str = "") -> bool:
    ...
    log_action("reject", "approval", req.id, f"{req.doc_type}:{req.doc_id}")
    return True
