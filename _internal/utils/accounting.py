...
ACCOUNT_TYPES: List[str] = ["asset", "liability", "equity", "revenue", "expense"]
...
def _ensure_default_mapping() -> None:
    existing: Dict[str, str] = {s.key: s.value for s in SystemSetting.query.all()}
    changed = False
...
def default_account_id(key: str) -> Optional[int]:
...
def set_default_account_id(key: str, account_id: Any) -> None:
    if key not in DEFAULT_ACCOUNT_MAP:
...
def next_entry_number(year_id: Optional[int]) -> str:
...
def _clean_year(year_id: Any) -> Optional[int]:
...
def make_entry(lines: List[Dict[str, Any]], date: Optional[datetime.date] = None, description: str = "",
               financial_year_id: Optional[int] = None, source: str = "manual",
               ref_type: Optional[str] = None, ref_id: Optional[int] = None,
               commit: bool = True) -> JournalEntry:
    total_dr = d0(0)
    total_cr = d0(0)
...
def delete_source_entries(source: str, ref_type: str, ref_id: Any, commit: bool = True) -> int:
...
def account_balance(account: Any, end_date: Optional[datetime.date] = None,
                    year_id: Optional[int] = None) -> float:
...
def ledger(account_id: int, start_date: Optional[datetime.date] = None,
           end_date: Optional[datetime.date] = None,
           year_id: Optional[int] = None) -> List[Dict[str, Any]]:
    year_id = _clean_year(year_id)
...
def trial_balance(year_id: Optional[int] = None,
...
def _revenue_balance(end_date: Optional[datetime.date] = None,
                     year_id: Optional[int] = None) -> Decimal:
    total = d0(0)
...
def _expense_balance(end_date: Optional[datetime.date] = None,
                     year_id: Optional[int] = None) -> Decimal:
    total = d0(0)
...
def income_statement(start_date: Optional[datetime.date] = None,
                     end_date: Optional[datetime.date] = None,
...
def balance_sheet(end_date: Optional[datetime.date] = None,
                  year_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    def add_section(items: List[Dict[str, Any]], total: Decimal, acc: Any, bal: float) -> Decimal:
...
def cash_flow(start_date: Optional[datetime.date] = None,
              end_date: Optional[datetime.date] = None,
              year_id: Optional[int] = None) -> Dict[str, Any]:
    year_id = _clean_year(year_id)
...
def post_invoice_entries(invoice: Any) -> Optional[JournalEntry]:
...
def post_payment_entries(source: str, ref_type: str, ref_id: int, amount: Any,
                         date: Optional[datetime.date] = None,
                         financial_year_id: Optional[int] = None,
                         is_receipt: bool = True, description: str = "") -> Optional[JournalEntry]:
    if not posting_enabled():
        return None
    amount = d0(amount)
    if amount <= 0:
...
def post_purchase_order_entries(po: Any) -> Optional[JournalEntry]:
    if not posting_enabled():
        return None
    try:
        exp_acc = default_account_id("acc_default_expense")
...
def post_contract_entries(contract: Any) -> Optional[JournalEntry]:
    if not posting_enabled():
        return None
    try:
        dr_acc = default_account_id("acc_default_receivable")
...
def reverse_entry(entry: JournalEntry, description: str = "إلغاء قيد") -> JournalEntry:
