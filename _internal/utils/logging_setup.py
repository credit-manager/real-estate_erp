...
APP_LOGGER_NAME: str = "dynamicpro"
SECURITY_LOGGER_NAME: str = "security"
DEFAULT_LEVEL: int = logging.INFO
_MAX_BYTES: int = 5 * 1024 * 1024   # 5 MB لكل ملف
_BACKUP_COUNT: int = 5               # يحتفظ بآخر 5 ملفات دوّارة
...
def configure_logging(level: Optional[Union[int, str]] = None) -> Logger:
    ...
    if not any(isinstance(h, RotatingFileHandler) for h in logger.handlers):
        try:
            ...
            logging.getLogger(APP_LOGGER_NAME).debug(
                "Failed to create log file handler; console-only logging", exc_info=True
            )
