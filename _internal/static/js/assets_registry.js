...
async function loadItems() {
  try {
    ...
    tbody.innerHTML = items.length ? items.map((a) => `
      ...
      </tr>`).join("") : `<tr><td colspan="10"><div class="empty-state">${t("assets.noItems")}</div></td></tr>`;
...
async function loadMeta() {
  ...
  if (!meta) return;
  fillSelect("it-category", meta.categories, t("common.select"));
...
function openItemModal(a) {
  ...
  document.getElementById("item-modal-title").textContent = a ? t("common.edit") : t("assets.newAsset");
  document.getElementById("it-code").value = a ? a.code : "";
  document.getElementById("it-name").value = a ? a.name : "";
  document.getElementById("it-kind").value = a ? a.kind : "asset";
