(function () {
  ...
  function gatherSectionData(section) {
    switch (section) {
      ...
      case "branding":
        return {
          override: c("set-branding-override"),
          customer_logo: v("set-customer-logo"),
        };
      case "appearance":
        return {
  ...
  var custFile = document.getElementById("set-customer-logo-file");
  var custUrl = document.getElementById("set-customer-logo-url");
  var custHidden = document.getElementById("set-customer-logo");
  var custPrevEl = document.getElementById("branding-customer-preview");

  function renderCustLogo(src) {
    if (!custPrevEl) return;
    custPrevEl.innerHTML = src
      ? '<img src="' + src + '" alt="logo" style="max-width:100%;max-height:100%;object-fit:contain;">'
      : '<span class="branding-logo-fallback">' + lt("settings.brandingNoLogo") + '</span>';
  }

  function clearCustLogo() {
    if (custHidden) custHidden.value = "";
    if (custUrl) custUrl.value = "";
    if (custFile) custFile.value = "";
    renderCustLogo("");
  }

  var clearBtn = document.getElementById("btn-clear-customer-logo");
  if (clearBtn) clearBtn.addEventListener("click", clearCustLogo);

  if (custFile) {
    custFile.addEventListener("change", function () {
      var file = custFile.files && custFile.files[0];
      if (!file) return;
      if (file.size > 1500000) {
        alert(lt("settings.logoTooLarge"));
        custFile.value = "";
        return;
      }
      ...
      reader.onload = function (ev) {
        ...
        if (custHidden) custHidden.value = src;
        if (custUrl) custUrl.value = "";
        renderCustLogo(src);
      };
  ...
  if (custUrl) {
    custUrl.addEventListener("input", function () {
      var val = custUrl.value.trim();
      if (custHidden && val) custHidden.value = val;
      renderCustLogo(val);
    });
  }
  ...
  function loadSettings() {
    fetch("/general-settings/api").then(function (res) { return res.json(); }).then(function (data) {
      ...
      // Branding
      var b = data.branding || {};
      var ownerPrev = document.getElementById("branding-owner-preview");
      if (ownerPrev) {
        ownerPrev.innerHTML = (b.owner_logo ? '<img src="' + b.owner_logo + '"' : '<img src="/static/img/logo-mark.svg"')
          + ' alt="logo" style="max-width:100%;max-height:100%;object-fit:contain;">';
      }
      var custLogo = b.customer_logo || "";
      setVal("set-customer-logo", custLogo);
      var cPrev = document.getElementById("branding-customer-preview");
      if (cPrev) {
        cPrev.innerHTML = custLogo
          ? '<img src="' + custLogo + '" alt="logo" style="max-width:100%;max-height:100%;object-fit:contain;">'
          : '<span class="branding-logo-fallback">' + lt("settings.brandingNoLogo") + '</span>';
      }
      setVal("set-customer-logo-url", custLogo.indexOf("http") === 0 ? custLogo : "");
      var bo = document.getElementById("set-branding-override");
      if (bo) bo.checked = !!b.override;
      var infoBox = document.getElementById("branding-customer-info");
      if (infoBox && b.customer) {
        infoBox.style.display = "";
        var cnm = document.getElementById("branding-customer-name");
        var cst = document.getElementById("branding-customer-status");
        var cen = document.getElementById("branding-customer-end");
        if (cnm) cnm.textContent = lt("settings.brandingCompany") + ": " + b.customer.name;
        if (cst) cst.textContent = lt("settings.brandingStatus") + ": " + lt("settings.brandingStatus_" + b.customer.status) + " (" + b.customer.status + ")";
        if (cen) cen.textContent = lt("settings.brandingUntil") + ": " + b.customer.valid_until;
      }
