...
_ALLOWED_TABLES = frozenset(DELETE_ORDER + list(PRESERVE_TABLES))


def _validate_table_name(table_name: str) -> None:
    """Raise ValueError if table_name is not in the known safe list."""
...
def _table_exists(raw_conn, table_name: str) -> bool:
    ...
    try:
        ...
        log.debug("Failed to check table existence for '%s'", table_name, exc_info=True)
        try:
            raw_conn.rollback()
        except Exception:
            pass
...
def _count_table(raw_conn, table_name: str) -> int:
    ...
    try:
        ...
        log.debug("Failed to count rows in table '%s'", table_name, exc_info=True)
        try:
            raw_conn.rollback()
        except Exception:
            pass
...
def _delete_table(raw_conn, table_name: str) -> int:
    """Delete all rows from a table. Returns number of deleted rows."""
    _validate_table_name(table_name)
    if not _table_exists(raw_conn, table_name):
        return 0
    try:
        ...
        log.warning("Failed to delete rows from table '%s'", table_name, exc_info=True)
        try:
            raw_conn.rollback()
        except Exception:
            pass
...
def get_reset_preview() -> Dict[str, Any]:
    """Get a preview of what would be deleted during factory reset."""
    try:
        db.session.rollback()
    except Exception:
        pass
...
def _seed_demo_data(raw_conn) -> None:
...
def factory_reset(seed_demo: bool = True) -> Dict[str, Any]:
    """Perform a factory reset, deleting all transactional data.
    
    Args:
        seed_demo: If True, seed demo data after reset.
        
    Returns:
        Dictionary with success status and deletion statistics.
    """
    ...
    try:
        ...
        if not is_sqlite:
            ...
            for seq_table in ["customers", "suppliers", "employees", "invoices",
                ...
                _validate_table_name(seq_table)
                try:
                    cur = raw_conn.cursor()
