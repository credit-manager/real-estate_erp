...
_rbac_cache: Dict[int, Dict[str, Any]] = {}
_rbac_lock = threading.Lock()
_RbacCacheTTL: int = 60


def _rbac_cache_get(master_user_id: int) -> Optional[Set[str]]:
    with _rbac_lock:
...
def _rbac_cache_set(master_user_id: int, perms: Set[str]) -> None:
    with _rbac_lock:
        _rbac_cache[master_user_id] = {"perms": perms, "ts": _time.time()}


def invalidate_rbac_cache(master_user_id: Optional[int] = None) -> None:
...
_ROLE_PERMISSIONS: Dict[str, Optional[List[str]]] = {
    "super_admin": None,
    "admin": [
...
def seed_roles_and_permissions() -> None:
    ...
    perms_by_code: Dict[str, MasterPermission] = {}
...
def user_permissions(master_user_id: int) -> Set[str]:
...
def has_permission(master_user_id: int, code: str) -> bool:
    return code in user_permissions(master_user_id)


def permitted(permission_codes: Set[str], required) -> bool:
...
def ensure_user_role_link(master_user_id: int, legacy_role_name: str) -> None:
