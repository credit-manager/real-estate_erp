...
def suspend_license_for_company(
    company_id: int,
    actor_email: Optional[str] = None,
    actor_id: Optional[int] = None,
    ip: Optional[str] = None,
    ...
    try:
        ...
        log.error("Failed to suspend licenses for company %d: %s", company_id, exc)
        db.session.rollback()
        return {"success": False, "message": f"فشل تعليق التراخيص: {str(exc)[:100]}"}
...
def revoke_license_for_company(
    company_id: int,
    actor_email: Optional[str] = None,
    actor_id: Optional[int] = None,
    ip: Optional[str] = None,
    ...
    try:
        ...
        log.error("Failed to revoke licenses for company %d: %s", company_id, exc)
        db.session.rollback()
        return {"success": False, "message": f"فشل إلغاء التراخيص: {str(exc)[:100]}"}
...
def renew_subscription_for_company(
    company_id: int,
    days: int = 365,
    actor_email: Optional[str] = None,
    actor_id: Optional[int] = None,
    ...
    try:
        ...
        log.error("Failed to renew subscription for company %d: %s", company_id, exc)
        db.session.rollback()
        return {"success": False, "message": f"فشل تجديد الاشتراك: {str(exc)[:100]}"}
