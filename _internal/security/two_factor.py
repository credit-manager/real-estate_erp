...
ISSUER: str = "ERP Control Center"
MFA_MAX_ATTEMPTS: int = config.MAX_LOGIN_ATTEMPTS
MFA_LOCK_SECONDS: int = 300
_MFA_FAILURES: Dict[int, Dict[str, Any]] = {}
...
def provisioning_uri(user_email: str, secret: str) -> str:
    """Generate a provisioning URI for TOTP enrollment."""
    return pyotp.TOTP(secret).provisioning_uri(name=user_email, issuer_name=ISSUER)


def _redis_mfa_store() -> Any:
    """Return the distributed MFA store; production never falls back silently."""
...
def _mfa_key(master_user_id: int) -> str:
    """Generate a Redis key for MFA operations."""
    digest = hashlib.sha256(str(master_user_id).encode("utf-8")).hexdigest()
    return f"dynamicpro:mfa:verify:{digest}"


def _mfa_attempt_allowed(master_user_id: int) -> bool:
    """Check if MFA attempt is allowed (not rate-limited)."""
...
def _register_mfa_failure(master_user_id: int) -> None:
    """Register an MFA failure and apply rate limiting."""
    store = _redis_mfa_store()
    if store is not None:
        key = _mfa_key(master_user_id)
...
def _reset_mfa_failures(master_user_id: int) -> None:
    """Reset MFA failure counter for the given user."""
    store = _redis_mfa_store()
    if store is not None:
        key = _mfa_key(master_user_id)
...
def enroll(master_user_id: int, user_email: str) -> Tuple[str, str]:
    """Enroll a user in MFA, returning (secret, provisioning_uri)."""
...
def verify_code(master_user_id: int, code: str) -> bool:
...
def is_enabled(master_user_id: int) -> bool:
    """Check if MFA is enabled for a master user."""
    from security.models import MasterTwoFactor
    mfa = MasterTwoFactor.query.filter_by(master_user_id=master_user_id).first()
    return bool(mfa and mfa.enabled_at)


def disable(master_user_id: int) -> bool:
    """Disable MFA for a master user."""
    from security.models import MasterTwoFactor
    mfa = MasterTwoFactor.query.filter_by(master_user_id=master_user_id).first()
    if mfa:
...
def generate_recovery_codes(master_user_id: int, count: int = 8) -> List[str]:
    """Generate recovery codes for a master user."""
    from security.models import MasterTwoFactor
    mfa = MasterTwoFactor.query.filter_by(master_user_id=master_user_id).first()
    if not mfa:
        return []
    ...
    for _ in range(count):
        # Use 8 bytes (64 bits) of entropy for 16 hex characters
    ...
    db.session.commit()
    _reset_mfa_failures(master_user_id)
    log.info("Generated %d recovery codes for master user %d", count, master_user_id)
    return codes


def verify_recovery_code(master_user_id: int, code: str) -> bool:
    """Verify a recovery code for a master user."""
    from security.models import MasterTwoFactor
    if not _mfa_attempt_allowed(master_user_id):
        return False
    mfa = MasterTwoFactor.query.filter_by(master_user_id=master_user_id).first()
...
def require_two_factor(master_user_id: int) -> bool:
    """Check if MFA is required for this user (super_admin without MFA still requires it)."""
