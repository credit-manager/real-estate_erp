...
def _validate(data, partial=False, existing=None):
    ...
    start = parse_date(data.get("start_date")) if "start_date" in data else (existing.start_date if existing else None)
    end = parse_date(data.get("end_date")) if "end_date" in data else (existing.end_date if existing else None)
...
def create_year():
    ...
    year = FinancialYear(
        ...
        start_date=parse_date(data.get("start_date")), end_date=parse_date(data.get("end_date")),
...
def update_year(year_id):
    ...
    target_start = parse_date(data["start_date"]) if "start_date" in data else year.start_date
    target_end = parse_date(data["end_date"]) if "end_date" in data else year.end_date
