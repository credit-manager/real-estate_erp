...
def list_roles() -> Response:
    roles: List[Role] = Role.query.order_by(Role.id.asc()).all()
...
def create_role() -> Response:
    ...
    err: Optional[str] = validate_input(data, [("name", {"required": True, "max_len": 200}), ("description", {"max_len": 500})])
    if err:
...
def update_role(role_id: int) -> Response:
    ...
    if "permissions" in data:
        raw: Dict[str, Any] = data["permissions"] or {}
...
def delete_role(role_id: int) -> Response:
    ...
    users_count: int = User.query.filter_by(role=role.name).count()
    if users_count:
        return jsonify({
