...
# ── Owner Protection ──────────────────────────────────────────
def is_owner(user_id: Optional[int] = None) -> bool:
...
def protect_owner_from_modify(target_user_id: int) -> bool:
...
# ── Activity Logging ──────────────────────────────────────────
def log_license_activity(action: str, details: str = "", user_id: Optional[int] = None, username: Optional[str] = None) -> None:
    try:
...
def create_owner_notification(title: str, message: str, notif_type: str = "user_action", related_user: Optional[str] = None) -> None:
    try:
...
def create_license() -> Response:
    ...
    required: List[str] = ["company_name", "contact_name", "contact_email", "license_type"]
    for field in required:
        if not data.get(field):
            return jsonify({"success": False, "message": f"Missing: {field}"}), 400

    lic_type: str = data["license_type"]
    if lic_type not in ("trial", "annual", "biennial"):
        return jsonify({"success": False, "message": "Invalid type"}), 400

    try:
        max_users: int = int(data.get("max_users", 10))
        max_users = max(1, min(max_users, 10000))
    except (TypeError, ValueError):
        max_users = 10

    duration = License.get_duration(lic_type)
    license_key: str = License.generate_key()
...
@require_page("settings", "edit")
@admin_required
def deactivate_license(lid: int) -> Response:
...
@require_page("settings", "edit")
@admin_required
def activate_license(lid: int) -> Response:
    lic = db.session.get(License, lid)
    if not lic:
        return jsonify({"success": False, "message": "Not found"}), 404
...
@require_page("settings", "edit")
@admin_required
def delete_license(lid: int) -> Response:
    lic = db.session.get(License, lid)
    if not lic:
        return jsonify({"success": False, "message": "Not found"}), 404
...
def list_activity() -> Response:
    try:
        ...
        limit: int = min(int(request.args.get("limit", 100)), 500)
...
def list_notifications() -> Response:
    try:
        ...
        unread: int = OwnerNotification.query.filter_by(is_read=False).count()
