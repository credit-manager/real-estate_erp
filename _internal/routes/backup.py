...
def _run_auto_backup_once(app):
    ...
    # الدفع السحابي بعد الكتابة المحلية (المحتوى مشفّر أصلاً إن وُجدت كلمة مرور)
    try:
        if settings.get_bool("backup_cloud_enabled", False):
            from utils.cloud_backup import push_backup
            with open(os.path.join(folder, filename), "rb") as fh:
                content = fh.read()
            ok, message = push_backup(content, filename, settings)
            if ok:
                settings.set("backup_cloud_last_status", "ok")
                settings.set("backup_cloud_last_error", "")
                settings.set("backup_cloud_last_at", now.isoformat())
            else:
                settings.set("backup_cloud_last_status", "error")
                settings.set("backup_cloud_last_error", message[:300])
                settings.set("backup_cloud_last_at", now.isoformat())
                log_action("error", "backup", None,
                           f"فشل الدفع السحابي للنسخة: {message}")
    except Exception as e:
        settings.set("backup_cloud_last_status", "error")
        settings.set("backup_cloud_last_error", str(e)[:300])
        log_action("error", "backup", None, f"خطأ في الدفع السحابي: {e}")
    db.session.commit()
        ...
        while True:
            try:
                ...
                try:
                    with app.app_context():
                        log_action("error", "backup", None,
                                   f"خطأ في النسخ الاحتياطي التلقائي: {e}")
                        db.session.commit()
                except Exception:
                    import logging
...
def get_cloud_settings():
    """إعدادات الدفع السحابي — لا تفصح عن الأسرار، فقط هل هي مضبوطة."""
    import utils.settings as settings
    data = settings.get_all()
    return jsonify({
        "success": True,
        "settings": {
            "backup_cloud_enabled": settings.get_bool("backup_cloud_enabled", False),
            "backup_cloud_type": data.get("backup_cloud_type", ""),
            "backup_cloud_url": data.get("backup_cloud_url", ""),
            "backup_cloud_user": data.get("backup_cloud_user", ""),
            "backup_cloud_bucket": data.get("backup_cloud_bucket", ""),
            "backup_cloud_region": data.get("backup_cloud_region", ""),
            "backup_cloud_secret_set": bool(data.get("backup_cloud_secret", "")),
            "backup_cloud_last_status": data.get("backup_cloud_last_status", ""),
            "backup_cloud_last_error": data.get("backup_cloud_last_error", ""),
            "backup_cloud_last_at": data.get("backup_cloud_last_at", ""),
        },
    })


@backup_bp.route("/cloud", methods=["POST"])
@require_api("backup", "edit")
def save_cloud_settings():
    import utils.settings as settings
    data = request.get_json(silent=True) or {}

    if "backup_cloud_enabled" in data:
        settings.set("backup_cloud_enabled", "1" if data["backup_cloud_enabled"] else "0")
    if "backup_cloud_type" in data:
        ctype = (data["backup_cloud_type"] or "").strip().lower()
        if ctype not in ("", "webdav", "s3"):
            return jsonify({"success": False, "error_key": "backup.cloudInvalidType"}), 400
        settings.set("backup_cloud_type", ctype)
    if "backup_cloud_url" in data:
        url = (data["backup_cloud_url"] or "").strip()
        if url:
            low = url.lower()
            if not (low.startswith("https://") or low.startswith("http://")):
                return jsonify({"success": False, "error_key": "backup.cloudInvalidUrl"}), 400
        settings.set("backup_cloud_url", url)
    if "backup_cloud_user" in data:
        settings.set("backup_cloud_user", (data["backup_cloud_user"] or "").strip())
    if "backup_cloud_secret" in data and (data["backup_cloud_secret"] or "").strip():
        # عند الحفظ بلا قيمة نُبقي السرّ الحالي
        settings.set("backup_cloud_secret", (data["backup_cloud_secret"] or "").strip())
    if "backup_cloud_bucket" in data:
        bucket = (data["backup_cloud_bucket"] or "").strip()
        if bucket and ("/" in bucket or "\\" in bucket):
            return jsonify({"success": False, "error_key": "backup.cloudInvalidBucket"}), 400
        settings.set("backup_cloud_bucket", bucket)
    if "backup_cloud_region" in data:
        settings.set("backup_cloud_region", (data["backup_cloud_region"] or "").strip())
    db.session.commit()

    if data.get("action") == "test":
        from utils.cloud_backup import push_backup
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        ok, message = push_backup(b"2TO test connection", f"_2to_cloud_test_{stamp}.txt", settings)
        if not ok:
            return jsonify({"success": False, "error_key": "backup.cloudTestFailed",
                            "detail": message}), 400
        log_action("edit", "backup", None, "اختبار اتصال سحابي ناجح")
        return jsonify({"success": True, "detail": message})

    log_action("edit", "backup", None, "تعديل إعدادات الدفع السحابي")
    return jsonify({"success": True})
