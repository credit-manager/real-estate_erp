...
# ==================== التحليلات التنفيذية المحلية ====================

@bi_bp.route("/analytics")
@require_api_any("view", ["reports", "finance", "sales", "inventory"])
def executive_analytics():
    """مؤشرات تنفيذية محلية: إيراد/مصروف/ذمم/مخزون/أداء — متاحة دون أي اعتماد خارجي."""
    from utils.analytics import compute_analytics
    return jsonify(compute_analytics())


@bi_bp.route("/alerts")
@require_api_any("view", ["reports", "finance", "sales", "inventory", "procurement", "rentals", "crm"])
def smart_alerts():
    """تنبيهات ذكية قابلة للتنفيذ: إعادة طلب، ذمم متأخرة، عقود تنتهي، أوامر/عروض معلقة."""
    from utils.analytics import compute_alerts
