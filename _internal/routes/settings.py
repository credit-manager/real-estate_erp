...
def _validate_section(section: str, data: Dict[str, Any]) -> Optional[str]:
...
def get_settings() -> Response:
    data: Dict[str, Any] = settings.get_all()
...
@require_api("settings", "edit")
def save_section_settings(section: str) -> Response:
    if section not in _VALID_SECTIONS:
        return jsonify({"success": False, "message": "invalid-section"}), 400
    data: Dict[str, Any] = request.get_json(silent=True) or {}
    err: Optional[str] = _validate_section(section, data)
    if err:
        return jsonify({"success": False, "message": err, "error_key": err}), 400
    section_keys: Set[str] = settings.SECTION_KEYS.get(section, set())
...
# ── POST save branding ────────────────────────────────────────
@settings_bp.route("/api/branding", methods=["POST"])
@require_api("settings", "edit")
def save_branding() -> Response:
    data: Dict[str, Any] = request.get_json(silent=True) or {}
    logo: str = (data.get("customer_logo") or "").strip()
    err: Optional[str] = branding.valid_logo(logo)
    if err:
        return jsonify({"success": False, "message": err, "error_key": err}), 400
    settings.set("branding_customer_logo", logo)
    settings.set("branding_override", "1" if data.get("override") else "0")
    db.session.commit()
    log_action("edit", "settings", None, "تعديل العلامة التجارية")
    return jsonify({"success": True, "message": "settings.saved", "branding": branding.info()})


# ── POST save company ─────────────────────────────────────────
@settings_bp.route("/api/company", methods=["POST"])
@require_api("settings", "edit")
def save_company() -> Response:
    data: Dict[str, Any] = request.get_json(silent=True) or {}
...
def save_layout() -> Response:
    data: Dict[str, Any] = request.get_json(silent=True) or {}
    layout: str = data.get("layout_style", "vertical")
...
@login_required
def update_exchange_rates() -> Response:
    """Update all currency exchange rates based on base currency."""
    from datetime import datetime, timezone
    data: Dict[str, Any] = request.get_json() or {}
    company_id = data.get("company_id")
    base_code: str = data.get("base_currency", "EGP")
    rates: Dict[str, Any] = data.get("rates", {})  # {currency_code: rate_vs_base}
    ...
    currencies: List[Currency] = Currency.query.filter_by(company_id=company.id, is_active=True).all()
...
_DOC_MODELS: Dict[str, Tuple[Any, str]] = {
...
@require_any_view
def next_number() -> Response:
    doc_type: str = request.args.get("type", "invoice")
    if doc_type not in _DOC_MODELS:
        return jsonify({"success": False, "message": "invalid-type"}), 400
    model, field = _DOC_MODELS[doc_type]
    prefix: str = settings.get(doc_type + "_prefix", "")
    if not prefix:
    ...
    for doc in model.query.all():
        value: str = getattr(doc, field, "") or ""
