...
DESKTOP_MODE = os.environ.get("DYNAMICPRO_DESKTOP", "0") == "1"

# ── بيانات المستخدم للنسخة المكتبية ──
# لا نكتب بيانات قابلة للتغيير داخل مجلد PyInstaller/Program Files.
if DESKTOP_MODE:
    _LOCAL_APPDATA = os.environ.get("LOCALAPPDATA") or os.path.expanduser("~")
    DATA_DIR = os.path.join(_LOCAL_APPDATA, "Dynamic Pro ERP", "Data")
...
_DB_PW_FILE = os.path.join(DATA_DIR, ".db_password")
...
# ── النسخة المكتبية: قاعدة SQLite محلية، بلا PostgreSQL ──
# النسخة السحابية تحتفظ بالكامل بإعداد PostgreSQL الحالي.
if DESKTOP_MODE:
    COMPANY_ID = ""
    COMPANY_PORT = ""
    IS_COMPANY_INSTANCE = False
    DB_NAME = os.path.join(DATA_DIR, "dynamicpro.db")
    SQLALCHEMY_DATABASE_URI = "sqlite:///" + DB_NAME.replace("\\", "/")
else:
...
# ── Per-company SECRET_KEY ──
if COMPANY_ID:
    _company_key_file = os.path.join(DATA_DIR, f".secret_key_{COMPANY_ID}")
